class CircleLine {
  ...
  float maxR = dist(0,0,width/2,height/2);
  float minR = 20;
  float deltaR = 1;
  float deltaAngle = 2;
  ...
  CircleLine() {
    this( random(minR, maxR/2),  random(PI*2) );
  }
  
  CircleLine(float r_ , float angle_){
    if(r_ == null) r_= random(minR, maxR/2);
    if(angle_ == null) angle_ = random(PI*2);
